...
**Schema** (row key = headhunter, by email): `Headhunter Name | Agency/Firm | Email | Introduced Opportunities | Notes | First Contact | Last Contact | # of Opportunities | Processed Message IDs`. Introduced Opportunities lists every distinct (company, position) that headhunter surfaced, semicolon-separated; note explicitly when a company/position was never named rather than leaving it blank. Sort by Last Contact descending, consistent with whatever sort rule the main tab uses.
